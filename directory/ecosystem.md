...
- [Beckn / ONDC](https://github.com/ONDC-Official) — India's open commerce protocol and the network built on it. Its signing spec carries no human-verification requirement, which makes an agent buyer-app protocol-legal today, gated only by network-registry onboarding. The largest open commerce network outside the crypto world.
...
- [ModelScope MCP Plaza](https://www.modelscope.cn/mcp) — Alibaba's MCP hub (魔搭社区), hosting Streamable-HTTP servers for tools unreachable from Western registries — Amap, 12306 rail, DingTalk, Alipay subscription. Free hosted endpoints. The largest MCP surface most English-language directories have never indexed.
